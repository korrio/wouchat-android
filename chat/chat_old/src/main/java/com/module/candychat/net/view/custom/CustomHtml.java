...
package com.module.candychat.net.view.custom;
...
public class CustomHtml {
	...
	public static Spanned fromHtml(String source, ICustomFontLoader fontLoader) {
		return fromHtml(source, fontLoader, null, null);
	}
	...
	public static Spanned fromHtml(String source, ICustomFontLoader fontLoader,
		...
		HtmlToSpannedConverter converter = new HtmlToSpannedConverter(source,
				fontLoader, imageGetter, tagHandler, parser);
	...
	static class HtmlToSpannedConverter implements ContentHandler {
		...
		public HtmlToSpannedConverter(String source,
				ICustomFontLoader fontLoader, ImageGetter imageGetter,
		...
		private static void startFont(SpannableStringBuilder text,
			...
			int len = text.length();
			text.setSpan(new Font(color, face, theSize), len, len,
		...
		private void endFont(SpannableStringBuilder text) {
			...
			if (where != len) {
				...
				if (f.mFace != null) {
					// Todo my Typeface HERE !!!
					/*-
					text.setSpan(new TypefaceSpan(f.mFace), where, len,
							Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
					 */

					text.setSpan(new CustomTypefaceSpan(f.mFace, mFontLoader),
							where, len, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
				}

				//if (f.mSize) {
					// Todo font size HERE !!!
					text.setSpan(new RelativeSizeSpan((float)f.mSize / 24),
							where, len, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
				//}
			}
		}
