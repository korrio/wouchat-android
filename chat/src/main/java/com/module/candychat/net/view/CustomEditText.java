...
public class CustomEditText extends EditText {
    ...
    private CalligraphyTypefaceSpan[] appliedTypefacesArray;

    public CalligraphyTypefaceSpan[] getAppliedTypefacesArray() {
    ...
    public void setFont(WFont wFont, int selectionStart, int selectionEnd) {
        this.currentTypeface = wFont.typeface;
        this.wFont = wFont;
        if(selectionStart > selectionEnd) {
        ...
        if(selectionEnd > selectionStart) {
            ...
            appliedTypefacesArray = (CalligraphyTypefaceSpan[])var14.getSpans(selectionStart, selectionEnd, CalligraphyTypefaceSpan.class);
            if(appliedTypefacesArray != null && appliedTypefacesArray.length > 0) {
                int typefaceStart = -1;
                int typefaceEnd = -1;
                Typeface beforeTypeface = null;
                Typeface afterTypeface = null;
                CalligraphyTypefaceSpan[] arr$ = appliedTypefacesArray;
                ...
                if(typefaceStart > -1) {
                    wFont.typeface = beforeTypeface;
                    var14.setSpan(new CalligraphyTypefaceSpan(wFont), typefaceStart, selectionStart, 33);
                }

                if(typefaceEnd > -1) {
                    wFont.typeface = afterTypeface;
                    var14.setSpan(new CalligraphyTypefaceSpan(wFont), selectionEnd, typefaceEnd, 33);
                }

                var14.setSpan(new CalligraphyTypefaceSpan(wFont), selectionStart, selectionEnd, 33);
            } else {
                var14.setSpan(new CalligraphyTypefaceSpan(wFont), selectionStart, selectionEnd, 33);
            }
    ...
    private class DWTextWatcher implements TextWatcher {
        ...
        private void handleInsertFontCharacter2(Editable editable, int position, ForegroundColorSpan currentForegroundColorSpan) {
            ...
            if(currentTypefaceSpan != null) {
                if(currentTypefaceSpan.getTypeface() != CustomEditText.this.currentTypeface) {
                    ...
                    if(position == fontfaceEndNextSpan) {
                        ...
                        if(oldTypeface != null && CustomEditText.this.currentTypeface == oldTypeface.getTypeface()) {
                            ...
                            editable.setSpan(new CalligraphyTypefaceSpan(currentTypefaceSpan.getwFont()), nextSpan, fontfaceEndNextSpan - this.appendTextLength, 34);
                            editable.setSpan(new CalligraphyTypefaceSpan(oldTypeface.getwFont()), position - this.appendTextLength, colorEndNextSpan1, 34);
                            return;
                        }
                    }
                    ...
                    if(position - this.appendTextLength < fontfaceEndNextSpan && nextSpan != fontfaceEndNextSpan) {
                        ...
                        WFont oldWFont = currentTypefaceSpan.getwFont();
                        if(nextSpan < position - this.appendTextLength) {
                            editable.setSpan(new CalligraphyTypefaceSpan(oldWFont), nextSpan, position - this.appendTextLength, 34);
                        }

                        editable.setSpan(new CalligraphyTypefaceSpan(CustomEditText.this.wFont), position - this.appendTextLength, position, 34);
                        if(position < fontfaceEndNextSpan) {
                            editable.setSpan(new CalligraphyTypefaceSpan(oldWFont), position, fontfaceEndNextSpan, 34);
                        }
                    } else {
                        editable.setSpan(new CalligraphyTypefaceSpan(CustomEditText.this.wFont), position - this.appendTextLength, fontfaceEndNextSpan, 34);
                    }
                }
                ...
                if(nextSpan1 != null) {
                    ...
                    if(CustomEditText.this.currentTypeface == nextSpan1.getTypeface()) {
                        editable.removeSpan(nextSpan1);
                        editable.setSpan(new CalligraphyTypefaceSpan(nextSpan1.getwFont()), position - this.appendTextLength, fontfaceEndNextSpan, 34);
                        return;
                    }
                }
